...
public:
  ...
  Payload_Argument(const Schema_Payload_Argument& payload_argument)
  {
      ...
      {
          index = std::atoi(payload_argument.value_identifier);
          json_pointer = "";
      }
      substring_start = payload_argument.substring_start;
      substring_end = payload_argument.substring_end;
  }
  std::string getValue(const rapidjson::Document::Document& d, const std::string path_header_value)
  {
      std::string str;
      if (json_pointer)
      {
...
public:
  ...
  Response(const Schema_Response_To_Return& resp)
  {
      ...
      if (tokenizedPayload.size() - payload_arguments != 1)
      {
          std::cerr<<"number of placeholders does not match number of arguments:"<<staticjson::to_pretty_json_string(resp)<<std::endl;
          abort();
      }
  ...
  std::string produce_payload(const rapidjson::Document::Document& d, const std::string path_header_value)
  {
      std::string payload;
      for (size_t index = 0; index < tokenizedPayload.size())
      {
